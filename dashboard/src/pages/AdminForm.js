...
const AdminForm = () => {
  ...
  const [newUser, setNewUser] = useState({
    ...
    username: "",
    password: "",
    UserType: "admin", // Assuming UserType should be included for admin users
  });
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      axios
        .get("http://localhost:3002/api/getadmins")
        .then((response) => {
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(
        `http://localhost:3002/api/editadmin/${selectedUser.userid}`,
        selectedUser
      );
      setUsers(
        users.map((user) =>
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      setNewUser({
        ...
        username: "",
        password: "",
        UserType: "admin", // Reset UserType as well
      });
      setOpenAdd(false);
